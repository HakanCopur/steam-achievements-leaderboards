...
public:
	...
	static void GetAchievementStatus(const FString& AchievementAPIName, bool& bUnlocked, int32& UnlockUnixTime, bool& bSuccess);
	...
	static void GetAchievementDisplayInfo(const FString& AchievementAPIName, FString& DisplayName, FString& Description, bool& bSuccess);
	...
	meta=(DisplayName="Get Stored Stat",
		  ToolTip="Reads a cached stat loaded via 'Request Current Stats'. Select the stat type; value is returned in the corresponding output.",
	...
	meta=(
		...
		ToolTip="Reads multiple cached stats (after Request Current Stats). Provide API names and read types; returns an array of results you can Break in Blueprint.",
		Keywords="steam stats get stored cached batch multiple array"
	))
	static void GetStoredStats(const TArray<FSAL_StatQuery>& StatsToGet, TArray<FSAL_StoredStat>& StatsOut, bool& bAllSucceeded);
};
